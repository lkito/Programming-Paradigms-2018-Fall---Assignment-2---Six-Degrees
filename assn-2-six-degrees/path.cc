...
//Prints actor-movie-actor connection.
void path::printConnection(string &firstActor, string &secondActor, film &movie){
  cout << firstActor << " was in \"" << movie.title << "\" (" 
          << startYear + movie.year << ") with " << secondActor << "." << endl << endl;
}

//Prints path from startPlayer to last one.
void path::print(){
  if(links.size() == 0) return;
  string firstActor = startPlayer;
  string secondActor;
  film movie;
  for(size_t i = 0; i < links.size(); i++){
    secondActor = links[i].player;
    movie = links[i].movie;
    printConnection(firstActor, secondActor, movie);
